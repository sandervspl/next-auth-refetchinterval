...
const IS_CI = Boolean(
  ...
  process.env.GITHUB_ACTIONS ?? process.env.GITLAB_CI ?? process.env.TF_BUILD ?? false,
);
...
const config = (phase, config) => {
  ...
  let cfg = {
    ...
    eslint: {
      ignoreDuringBuilds: IS_CI, // we run a separate lint step in CI
    },
    typescript: {
      ignoreBuildErrors: IS_CI, // we run a separate typecheck step in CI
    },
    async headers() {
      return [
